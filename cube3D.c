...
void  init_window(t_object_on_scene *objects)
{
  ...
  objects->win = mlx_new_window(objects->mlx, objects->s_value_from_map.resolution_x,
  objects->s_value_from_map.resolution_y, "Cube3D");
  take_position_player(objects);
  cardinal_points(objects);
  quantity_sprite1(objects);
  objects->speed = 5;
  objects->window.img = mlx_new_image(objects->mlx, objects->s_value_from_map.resolution_x,
